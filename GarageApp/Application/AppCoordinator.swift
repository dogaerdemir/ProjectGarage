...
final class AppCoordinator: BaseCoordinator {
    ...
    override func start() {
        ...
        tabBarController = tabs; injectDependencies(in: tabs); window.rootViewController = tabs; window.makeKeyAndVisible()
        Task {
            do {
    ...
    private func showVehicles(from presenter: UIViewController) {
        ...
        list.onDelete = { [weak self, weak list] vehicle in
            list?.confirm(title: "Aracı Sil", message: "\(vehicle.nickname) ve tüm kayıtları, belgeleri ve hatırlatmaları kalıcı olarak silinecek.", destructiveTitle: "Sil") {
                Task { do { try await self?.deleteVehicle(vehicle); list?.tableView.reloadData() } catch { list?.presentError(error) } }
            }
        }
        list.onArchive = { [weak self, weak list] vehicle in
            var archived = vehicle; archived.isArchived.toggle(); archived.updatedAt = .now
            Task { do { try await self?.container.vehicleRepository.save(archived); await self?.container.session.dataChanged(); list?.reload() } catch { list?.presentError(error) } }
        }
    ...
    private func chooseRecordType(from presenter: UIViewController) {
        ...
        let alert = UIAlertController(title: "Kayıt Türü", message: nil, preferredStyle: .actionSheet)
        RecordType.allCases.forEach { type in alert.addAction(UIAlertAction(title: type.displayName, style: .default) { _ in self.showRecordEditor(type: type, from: presenter) }) }
        alert.addAction(UIAlertAction(title: "Vazgeç", style: .cancel)); presenter.present(alert, animated: true)
    }
    ...
    private func showRecordDetail(_ record: VehicleRecord, from presenter: UIViewController) {
        Task {
            let items = (try? await container.recordRepository.lineItems(recordID: record.id)) ?? []
            let detail = RecordDetailViewController(record: record, lineItems: items)
            detail.onEdit = { [weak self, weak detail] in guard let self, let detail else { return }; self.showRecordEditor(type: record.recordType, existing: record, from: detail) }
            detail.onDelete = { [weak self, weak detail] in
                detail?.confirm(title: "Kaydı Sil", message: "Kayıt ve ilişkili belgeler kalıcı olarak silinecek.", destructiveTitle: "Sil") {
                    ...
                    Task { do { try await DeleteRecordUseCase(repository: self.container.recordRepository, documentRepository: self.container.documentRepository, storage: self.container.fileStorageService).execute(recordID: record.id, vehicleID: record.vehicleID); await self.container.session.dataChanged(); detail?.navigationController?.popViewController(animated: true) } catch { detail?.presentError(error) } }
                }
            }
